...
export function TechStackCarousel() {
  return (
    <section className="py-16 sm:py-24 bg-background overflow-hidden">
      ...
      <div className="mt-12 relative flex flex-col gap-6 overflow-hidden">
        <div className="flex animate-scroll-x" style={{ animationDuration: '60s' }}>
          {[...technologies, ...technologies].map((icon, index) => (
            <div
              key={`top-${index}`}
              className="flex-shrink-0 w-32 h-32 flex items-center justify-center rounded-lg bg-gradient-to-br from-secondary to-background border border-border mx-3"
            >
              <div className="text-foreground/80">{icon}</div>
            </div>
          ))}
        </div>
        <div className="flex animate-scroll-x-reverse" style={{ animationDuration: '60s' }}>
          {[...technologies, ...technologies].map((icon, index) => (
            <div
              key={`bottom-${index}`}
              className="flex-shrink-0 w-32 h-32 flex items-center justify-center rounded-lg bg-gradient-to-br from-secondary to-background border border-border mx-3"
